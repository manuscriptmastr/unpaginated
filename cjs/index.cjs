...
var andThen =
/*#__PURE__*/
_curry2_1(function andThen(f, p) {
  _assertPromise_1('andThen', p);
...
var ap =
/*#__PURE__*/
_curry2_1(function ap(applyF, applyX) {
...
var liftN =
/*#__PURE__*/
_curry2_1(function liftN(arity, fn) {
...
var _xchain =
/*#__PURE__*/
_curry2_1(function _xchain(f, xf) {
...
_curry2_1(
/*#__PURE__*/
_dispatchable_1(['fantasy-land/chain', 'chain'], _xchain_1, function chain(fn, monad) {
...
var hasPath =
/*#__PURE__*/
_curry2_1(function hasPath(_path, obj) {
  if (_path.length === 0 || isNil_1(obj)) {
    return false;
  }
  ...
  while (idx < _path.length) {
    if (!isNil_1(val) && _has_1(_path[idx], val)) {
      val = val[_path[idx]];
      idx += 1;
    } else {
      return false;
...
var has =
/*#__PURE__*/
_curry2_1(function has(prop, obj) {
...
var is =
/*#__PURE__*/
_curry2_1(function is(Ctor, val) {
...
var pipeWith =
/*#__PURE__*/
_curry2_1(function pipeWith(xf, list) {
  if (list.length <= 0) {
    return identity_1;
  }
...
var when =
/*#__PURE__*/
_curry3_1(function when(pred, whenTrueFn, x) {
...
var unless =
/*#__PURE__*/
_curry3_1(function unless(pred, whenFalseFn, x) {
  return pred(x) ? x : whenFalseFn(x);
});

var unless_1 = unless;

const chainRec = curry_1(async (fn, acc) => {
...
const withTotal = when_1(Array.isArray, data => ({ data, total: undefined }));

const validateWith = curry_1((pred, message) => unless_1(pred, () => raise(new TypeError(message))));
const raise = err => { throw err };
const promisify = fn => async (...args) => fn(...args);

const isDataTotalObject = both_1(has_1('data'), has_1('total'));
const isCursorObject = both_1(has_1('data'), has_1('cursor'));
const isActionableCursor = either_1(both_1(is_1(String), s => !!s.length), is_1(Number));
const isValidPageValue = either_1(Array.isArray, isDataTotalObject);
    ...
    total === undefined
      ? fn(page, limit).then(({ data: d, total: t }) =>
          d.length === 0 || d.length < limit
            ? done(Promise.all([...data, Promise.resolve(d)]).then(unnest_1))
            : next({ data: [...data, Promise.resolve(d)], page: page + 1, limit: d.length, total: t })
        )
      : page * limit < total
        ? next({ data: [...data, fn(page, limit).then(prop_1('data'))], page: page + 1, limit, total })
        : done(Promise.all([...data, fn(page, limit).then(prop_1('data'))]).then(unnest_1))
  ,
  { data: [], page: 1, total: undefined, limit: 0 }
);

const pageMiddleware = fn => pipeWith_1(andThen_1, [
  promisify(fn),
  validateWith(isValidPageValue, 'Function must return an array of data or an object with data and total'),
  withTotal
]);

const byPage = pipe_1(unary_1, pageMiddleware, _page);

const offset = curry_1((limit, page) => (page - 1) * limit + 0);
const byOffset = pipe_1(fn => pipe_1(flip_1(offset), fn), pageMiddleware, _page);
...
const cursorMiddleware = fn => pipeWith_1(andThen_1, [
  promisify(fn),
  validateWith(isCursorObject, 'Function must return an object with data and cursor')
]);

const byCursor = pipe_1(cursorMiddleware, _cursor);

exports.byCursor = byCursor;
exports.byOffset = byOffset;
exports.byPage = byPage;
exports.default = byPage;
